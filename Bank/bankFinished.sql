...
--
-- Host: 127.0.0.1
-- Czas generowania: 28 Maj 2021, 18:09
...
--
-- Struktura widoku `szacowana_wypłata`
--
DROP TABLE IF EXISTS `szacowana_wypłata`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `szacowana_wypłata`  AS SELECT `p`.`ID` AS `ID`, `d`.`Imię` AS `Imię`, `d`.`Nazwisko` AS `Nazwisko`, `w`.`PensjaPodstawowa`+ coalesce(`w`.`OsiągnietaPremia`,0) AS `Wypłata` FROM ((`pracownik` `p` join `wypłata` `w` on(`p`.`ID` = `w`.`Pracownik_ID`)) join `dane_osobowe` `d` on(`d`.`ID` = `p`.`DaneOsobowe_ID`)) ;
