...
--
-- Host: 127.0.0.1
-- Czas generowania: 26 Maj 2021, 00:32
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `zrealizowane_transakcje`  AS  select `tr`.`ID` AS `ID`,`rot`.`Nazwa` AS `Nazwa`,`tr`.`Wartość` AS `Wartość`,`dos`.`Imię` AS `Imię`,`dos`.`Nazwisko` AS `Nazwisko`,`lok`.`Województwo` AS `Województwo`,`lok`.`Miasto` AS `Miasto`,`lok`.`KodPocztowy` AS `KodPocztowy`,`lok`.`Ulica` AS `Ulica`,`lok`.`NumerMieszkania` AS `NumerMieszkania` from (((`transakcje` `tr` join `rodzaj_transakcji` `rot` on(`tr`.`RodzajTransakcji_ID` = `rot`.`ID`)) join `dane_osobowe` `dos` on(`tr`.`Klient_ID` = `dos`.`ID`)) join `lokalizacja` `lok` on(`tr`.`Lokalizacja_ID` = `lok`.`ID`)) ;
